...
We approached this as a binary classification issue and used heterogeneous tabular features, including demographic information, medical history, clinical measurements, cognitive assessments, and reported symptoms. To address challenges such as feature scaling, moderate class imbalance, and nonlinear feature interactions, two deep learning models were implemented via PyTorch: a baseline feedforward multilayer perceptron (MLP) and a transformer-based TabTransformer model that leveraged self-attention to model relationships between features.

By way of feature engineering, categorical embeddings, weighted loss functions, and early stopping strategies, our goal was to conduct comparative experimentation and evaluate the effectiveness of deep learning methodology, specifically for tabular healthcare prediction using metrics like accuracy, F1-score, and ROC-AUC.
...
Running notebook sequentially produces:
